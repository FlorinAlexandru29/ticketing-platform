...
export default function CreateEventShell() {
  ...
  async function generateEventDescription() {
    if (
      ...
      alert(
        "Please fill in the event name, type, start date, venue and select at least one artist."
      );
      return;
    }
    ...
    try {
      ...
      const venueForPrompt = `${selectedVenue.name}, ${
        cityValue || cityQuery || ""
  ...
  function handleSelectArtist(artist: Artist) {
    ...
    (async () => {
      ...
      setSelectedArtists((prev) =>
        prev.map((x) => (x.id === artist.id ? { ...x, albums, albumsLoading: false } : x))
      );
    })();
  }
  ...
  const [ticketType, setTicketType] = useState<
    { id: number; category: string; description: string; price: string; nrOfTickets: string }[]
  >([]);
  ...
  // Venue search hooked to your /api/places/search
  useEffect(() => {
  ...
  const [creating, setCreating] = useState(false); // loading state for submit
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (selectedArtists.some((a) => !a.time))
      missing.push("Artist play time for all artists");
    ...
    if (!posterUrl) {
      const proceed = confirm(
        "No poster selected or generated. Do you want to continue without a poster?"
      );
      if (!proceed) return;
    } else {
    ...
    try {
      ...
      // Success: silent redirect to dashboard list
        ...
        <div className="flex justify-center p-4 sm:p-8">
          <div className="card bg-base-300 border-base-300 rounded-box my-4 card-lg shadow-sm w-full max-w-112 sm:max-w-3xl">
            <div className="card-body p-6 md:p-8 lg:p-12">
              <h2 className="card-title self-center mb-4 whitespace-nowrap text-[var(--heading-sz)]">
                Event Details
              </h2>
                ...
                <div className="flex flex-col gap-4">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <fieldset className="fieldset">
                      ...
                      <details className="dropdown" open={editingCity}>
                        <summary className="input p-0 focus-within:outline-none w-full border-none focus-within:shadow-none">
                          <input
                            ...
                            placeholder={
                              countryCode
                                ? "Start typing a city (min 2 chars)"
                      ...
                      {cityError && (
                        <div className="px-4 -mt-2 text-error text-sm w-3/4">{cityError}</div>
                      )}
                    </fieldset>
                    ...
                    <fieldset className="fieldset">
                      ...
                      {venueError && (
                        <div className="px-1 mt-1 text-error text-sm">{venueError}</div>
                      )}
                    </fieldset>
                  </div>
                  ...
                  <div
                    className={`${
                      mapExpanded ? "h-64 w-full" : "h-32 w-1/2"
                    } transition-all transition-discrete duration-300 rounded-lg ${
                      mapsEmbedUrl ? "relative" : "hidden"
                    } text-base-content border border-base-200 overflow-hidden`}
                  >
                    {mapsEmbedUrl && (
                      <>
                        <iframe
                ...
                <fieldset className="fieldset bg-base-200 border-base-200 rounded-box w-full border p-4 gap-6">
                  ...
                  <div className={`dropdown ${editingArtist ? "dropdown-open" : ""}`}>
                    ...
                    {editingArtist && artistQuery.length > 1 && (
                      <ul className="menu dropdown-content bg-base-100 rounded-box w-80 p-2 shadow-sm max-h-72 overflow-auto" style={{ flexFlow: "column" }}>
                        {artistSearchLoading && (
                          <li className="disabled">
                            <a>
                              <span className="loading loading-spinner mr-2" />
                  ...
                  <ul className="list bg-base-100 rounded-box shadow-md mt-3">
                    {selectedArtists.map((a) => (
                      <li
                        key={a.id}
                        className="list-row grid-cols-[1fr_1fr_auto] p-2 h-16 items-center"
                      >
                        ...
                        <button
                          ...
                          onClick={() =>
                            setSelectedArtists((prev) => prev.filter((x) => x.id !== a.id))
                          }
                        >
                          <FontAwesomeIcon icon={faMinus} />
                        </button>
                  ...
                  <ul className="list bg-base-100 rounded-box shadow-md">
                    {ticketType.map((ticket) => (
                      <li className="list-row grid-cols-none items-center" key={ticket.id}>
                        <p> {ticket.category} </p>
                        <p>
                          <FontAwesomeIcon icon={faHashtag} /> {ticket.nrOfTickets}
                        </p>
                        <p>
                          <FontAwesomeIcon icon={faMoneyBill1Wave} /> {ticket.price}{" "}
                        </p>
                        <p className="max-h-20 overflow-y-auto">
                          <FontAwesomeIcon icon={faComment} /> {ticket.description}{" "}
                        </p>
                        <button
                          className="btn btn-circle btn-error ml-auto"
                          onClick={() => {
