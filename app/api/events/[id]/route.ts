...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  const auth = await canEdit(params.id);
  ...
  try {
    const updated = await prisma.$transaction(async (tx) => {
      ...
      if (Object.keys(data).length) {
        await tx.event.update({ where: { id: params.id }, data });
      }
      ...
      if (input.data.ticketTiers) {
        ...
        for (const t of input.data.ticketTiers) {
          ...
          if (t.quantity < curQ) {
            // enforce "no lowering quantity"
            await tx.ticketTier.update({
              where: { id: t.id },
              data: { priceCents: t.priceCents } // only apply price
            });
          } else {
            await tx.ticketTier.update({
              where: { id: t.id },
